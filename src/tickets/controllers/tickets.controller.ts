...
export class TicketsController {
  ...
  @ApiOperation({ summary: 'Update a ticket by ID (Superadmin and Support only)' })
  ...
  @Patch(':id/assign')
  @Roles(UserRoleEnum.SUPERADMIN, UserRoleEnum.SUPPORT)
  @ApiOperation({ summary: 'Assign a ticket to a support user (Superadmin and Support only)' })
  @ApiResponse({ status: 200, description: 'The ticket has been successfully assigned.' })
  @ApiResponse({ status: 404, description: 'Ticket not found.' })
  @ApiResponse({ status: 403, description: 'Forbidden.' })
  assignToSupport(@Param('id') ticketId: string, @Body() assignToSupportDto: AssignToSupportDto) {
